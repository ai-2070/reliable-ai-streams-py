...
uv run mypy src/l0                   # strict; advisory (see below)
uv run ruff check src/l0 tests       # advisory (see below)
uv run ruff format src/l0 tests
```
...
- **Never run `ruff check --fix` / `ruff format` repo-wide** and never bulk-fix these findings. That produces enormous unrelated diffs.
- Keep *your own* diff clean: run the checks and only address findings on lines you touched.
...
src/l0/
  ...
  eventsourcing/    # recorder, replayer, store, sourcing, adapters
